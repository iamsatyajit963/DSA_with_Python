...
Example Input
Input 1: 	A = [1, 0, 1, 0, 1]	 B = 1 
Input 2:	 A = [0, 0, 0, 1, 1, 0, 1]	 B = 0 
Example Output
Output 1:	 [1, 2, 3]
...
def alternating_subarray(A, B):
    indices = []
    for i in range(len(A)):
        if i - B >= 0 and i + B < len(A):
            subarray = A[i-B:i+B+1]
...
A = [1, 0, 1, 0, 1]
B = 1
print(alternating_subarray(A, B))
